...
func run()  {
	...
	for i :=0 ; i < num; i ++ {
		go func() {
			_ ,err:= HttpBidRequest(adRequest, 3000)
			if err != nil {
				fmt.Println("err:", err)
			} else {
...
func HttpBidRequest(bidRequest *AdRequest, bidTimeOut int) (string, error) {
	...
	var apiUrl = "http://192.168.41.66/tradplus/__bidding_api/mintegral.php"
